...
dt = 0.01 # secondes
```
**Question** Définir une liste `py§temps` qui contient tous les instants allant de `py§0` (inclus) à `py§duree` (inclus) pour un pas de `py§dt`.

**Question** Écrire une fonction `py§euler` qui, étant donné deux arguments correspondant aux valeurs de $\theta$ et $\dot \theta$ au temps $t$, retourne les valeurs de $\theta$ et $\dot \theta$ actualisées pour le temps $t + \mathrm{d} t$.
...
**Question** On souhaite écrire une fonction `py§simule` qui prends en entrée deux arguments `py§theta_0` et `py§theta_point_0` correspondant aux conditions initiales du pendule, et qui renvoie deux listes correspondant aux valeurs successives de $\theta$ et de&nbsp;$\dot \theta$ durant la simulation. Les listes doivent avoir la même longueur de la liste `py§temps` définie précédemment. 
```python
...
1. Comment initialiser les listes `py§liste_theta` et `py§liste_theta_point`&nbsp;?
...
**Question** Modifier la fonction `py§euler` en une fonction `py§euler_harmo` pour simuler un oscillateur harmonique, et tracer les trajectoires pour des vitesses initiales nulles et des angles initiaux allant de $0$ à $2$ avec un pas de $0.1$.

Revenons maintenant à l'équation de mouvement initial et la fonction `py§euler` correspondante.
...
**Question** Écrire une fonction `py§energie` en supposant que le produit $m l^2$ est égal à $1$ dans les unités utilisées.
...
**Question** Modifier la fonction `py§euler` précédente pour obtenir une nouvelle fonction `py§euler_2` correspondant à la méthode semi-implicite de votre choix (la modification a effectuer étant, dans chaque cas, vraiment très simple).

**Question** Transformer de même la fonction `py§simule` en une fonction `py§simule_2` qui utilise `py§euler_2`.
