...
Si l'on veut la deuxième plus petite valeur, c'est à peine plus compliqué. On considère deux variables `py§m1` et `py§m2` qui correspondent respectivement à la plus petite valeur et la deuxième plus petite valeur.
...
Le programme correspondant est très simple. Contrairement à Quicksort, on peut se contenter d'une unique boucle `py§while`, puisqu'après avoir choisi un pivot, on ne doit traiter qu'un côté et non les deux&nbsp;:
