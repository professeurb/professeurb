...
import DichoBox from './Dicho.jsx'
...
- Si une propriété est vérifiée en entrant dans une boucle,
...
Pour prouver qu'il s'agit bien d'un invariant de boucle, nous allons supposer que notre propriété est vérifiée au début du corps de la boucle, puis prouver que c'est encore le cas à la fin.
...
Sinon, on a `py§t[m] != v`, et deux cas sont possibles&nbsp;:
...
- sinon, on a `py§t[m] > v` et donc si `py§v in t[a:b + 1]`, alors `py§v in t[a:m]`. Ainsi, en posant `py§b = m - 1`, on a `py§v in t[a:b + 1]`.
...
Vérifions donc que notre invariant est vrai avant d'entrer dans la boucle. À ce moment, on a `py§a == 0` et `py§b == len(t) - 1` et donc `py§t[a:b + 1] == t[0:len(t)] == t`. Notre invariant correspond alors à la tautologie
...
Pour résumer, notre propriété est vérifiée en entrée de boucle et c'est un invariant de la boucle. En conséquent, elle reste vérifiée en sortant de la boucle (quelque soit le nombre d'itérations effectué). De plus, à ce moment, on a `py§a > b` et donc `py§t[a:b + 1] = []` et on peut donc affirmer que $\mathtt{v} \in \mathtt{t} \implies \mathtt{v} \in \mathtt{[ \, ]}$.
