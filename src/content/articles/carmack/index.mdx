...
Notons tout d'abord que ce code manipule des flottants de type `cpp§float`, codés sur 32 bits, ce qui était la norme à l'époque. La répartition est la suivante&nbsp;:
* 1 bit pour le signe&nbsp;;
* 8 bits pour l'exposant auquel on ajoute 127&nbsp;;
* 23 bits pour la mantisse.

Notons que les flottants codés sur 64 bits, courants de nos jours, ont en langage C le type `cpp§double`.

Dans ces trois lignes, la première permet de considérer le codage du flottant&nbsp;`cpp§y` comme… un entier de 32 bits. Elle se comprend ainsi&nbsp;: `cpp§&y` est l'adresse où est stockée la représentation de `cpp§y`. En appliquant `cpp§(long *)`, on fait croire à l'ordinateur qu'à cette adresse se trouve un entier (un entier long, pour être précis, sur 32 bits). Ayant maintenant une adresse où est stockée (soi disant) un entier, on récupère la valeur de cet entier avec l'indirection `cpp§*`.
